  ...
  <div>
    ...
    <div id="example">
      <table id="list" class="table table-striped table-bordered table-hover">
        ...
        <tbody id="contacts">
          <tr v-for="contact in contactlist.contacts" v-bind:key="contact.no">
            <td>{{ contact.name }}</td>
            <td>{{ contact.tel }}</td>
            <td>{{ contact.address }}</td>
            <td>
              <img class="thumbnail" v-bind:src="contact.photo" v-on:click="editPhoto(contact.no)">
            </td>
            <td>
...
export default {
  ...
  methods: {
    ...
    leave(el, done) {
      ...
      Velocity(el, { translateX: '20px', opacity: 1 }, { duration: 200, loop: 2 })
      Velocity(el, { translateX: '0px', opacity: 1 }, { duration: 200 })
      Velocity(el, { translateX: '100px', opacity: 0 }, { duration: 200 })
    }
  }
}
</script>
