  ...
  <div class="modal">
    <div class="form" @keyup.esc="cancelEvent">
      ...
      <div class="form-group">
        <label>이름</label>
        <input type="text" name="name" class="long" v-model="contact.name" ref="name" placeholder="이름을 입력해주세요" />
      </div>
      <div class="form-group">
        <label>전화번호</label>
        <input type="text" name="tel" class="long" v-model="contact.tel" placeholder="전화번호를 입력해주세요" />
      </div>
      <div class="form-group">
        <label>주소</label>
        <input type="text" name="address" class="long" v-model="contact.address" placeholder="주소를 입력해주세요" />
      </div>
      <div class="form-group">
        <label>&nbsp;</label>
        <input type="button" name="btn btn-primary" v-bind:value="btnText" @click="submitEvent()" />
        <input type="button" name="btn btn-primary" value="취소" @click="cancelEvent()" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: _.extend({
      ...
      headingText() {
        if(this.mode != 'update') return "연락처 추가";
        return "연락처 변경";
      }
